...
export async function cmdChannelAdd(rawChannel) {
...
export async function cmdChannelRemove(rawChannel) {
  const channel = validateChannel(rawChannel);
  const config = await loadConfig();
  const channels = config.channels ?? {};
  if (!channels[channel]) {
...
export async function cmdChannelList() {
...
export async function cmdChannelTest(rawChannel) {
  if (rawChannel) {
    const channel = validateChannel(rawChannel);
...
function getRequiredCredentials(channel) {
  switch (channel) {
    case "telegram":
      return ["bot_token"];
    case "discord":
