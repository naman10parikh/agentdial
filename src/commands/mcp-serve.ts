...
// ── MCP Protocol Types (inline — no SDK dependency) ──
...
function validateChannel(raw: unknown): ChannelType {
...
async function handleIdentityStatus(): Promise<ToolResult> {
...
async function handleIdentitySetup(
  args: Record<string, unknown>,
): Promise<ToolResult> {
...
async function handleChannelAdd(
  ...
  const creds = (args["credentials"] as Record<string, string>) ?? {};
...
async function handleChannelList(): Promise<ToolResult> {
  const configured = await listConfiguredChannels();
  const config = await loadConfig();
...
async function handleGatewayStart(
  ...
  const port = (args["port"] as number) ?? 3141;
  const agentUrl = args["agent_url"] as string | undefined;
  ...
  return new Promise((resolvePromise) => {
    const server = createServer(async (req, res) => {
      ...
      if (req.url === "/message" && req.method === "POST") {
        try {
          const body = await readBody(req);
          const msg = GatewayMessageSchema.parse(JSON.parse(body) as unknown);
          const response = await routeMessage(msg, finalAgentUrl);
          res.writeHead(200, { "Content-Type": "application/json" });
...
function handleGatewayStop(): ToolResult {
  if (!gatewayServer) {
...
export async function cmdMcpServe(): Promise<void> {
