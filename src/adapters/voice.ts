...
export class VoiceAdapter implements ChannelAdapter {
  ...
  private phoneNumber = "";
  private webhookUrl = "";
  private voiceProvider: VoiceProvider = "openai-realtime";
  ...
  async send(_to: string, _response: GatewayResponse): Promise<void> {
   ...
   */
  async handleWebhook(payload: TwilioVoicePayload): Promise<string> {
...
function buildGatherTwiml(webhookUrl: string, sayText: string): string {
...
function buildSayTwiml(text: string): string {
  return [
    '<?xml version="1.0" encoding="UTF-8"?>',
    "<Response>",
...
function isValidProvider(value: string): value is VoiceProvider {
