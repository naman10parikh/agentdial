...
interface StatusOutput {
  identity: { name: string; version: string; channels: number } | null;
  channels: Array<{
    name: string;
    enabled: boolean;
    credentials: number;
  }>;
  gateway: { configured: boolean; port: number; agentUrl: string | null };
}
...
export async function cmdStatus(opts: { json?: boolean }): Promise<void> {
  ...
  // Identity
  let identityStatus: StatusOutput["identity"] = null;
  ...
  // Channels
  const channelStatuses: StatusOutput["channels"] = [];
